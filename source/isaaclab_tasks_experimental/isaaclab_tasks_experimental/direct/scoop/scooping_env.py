...
class ScoopingEnv(DirectRLEnv):
    ...
    def _register_newton_mpm_callbacks(self) -> None:
        ...
        h2 = NewtonManager.register_callback(
            _on_physics_ready, PhysicsEvent.PHYSICS_READY, name="scooping_mpm_physics_ready"
        )
    ...
    def _add_particle_grid(self, builder, voxel_size: float, origin: np.ndarray) -> None:
        ...
        builder.add_particle_grid(
            pos=wp.vec3(float(particle_lo[0]), float(particle_lo[1]), float(particle_lo[2])),
    ...
    # -------------------------------------------------------------------------
    # Visual container (USD, no collision — collision is in Newton builder)
    # -------------------------------------------------------------------------
    ...
    def _spawn_visual_container(self) -> None:
        ...
        for name, cfg, translation in pieces:
            cfg.func(f"{container_path}/{name}", cfg, translation=translation)
